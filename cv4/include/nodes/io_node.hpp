...
namespace nodes {
    class IoNode : public rclcpp::Node {
        rclcpp::Subscription<std_msgs::msg::UInt8>::SharedPtr button_sub_; // Subscriber for button press messages
        rclcpp::Publisher<std_msgs::msg::UInt8MultiArray>::SharedPtr button_pub_; // Publisher for leds
    ...
    public:
        ...
        IoNode(): Node("button_subscriber"), start_time_(this->now()) {
            button_sub_ = this->create_subscription<std_msgs::msg::UInt8>(
                ...
                std::bind(&IoNode::button_callback, this, std::placeholders::_1));
    ...
    private:
        rclcpp::Time start_time_; // time of start
        int button_pressed_ = -1;   // Variable to store the last received button press value
